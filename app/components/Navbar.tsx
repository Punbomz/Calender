...
import { Menu, Calendar, User, Plus, Trash2, CheckSquare, Inbox, ListTodo } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const activeTasks = tasks.filter((t: any) => !t.isFinished);
        const completed = tasks.filter((t: any) => t.isFinished);
      ...
      <>
              ...
              <div className="flex-1 overflow-y-auto p-4 pb-24">
                <div className="space-y-1 mb-6">
                  <button
                    ...
                    className={`hover: cursor-pointer w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                      !view ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-zinc-800'
                    }`}
                  >
                    <Inbox size={18} />
                    <span>All</span>
                ...
                <div className="mb-6">
                  ...
                  {categories.map((category) => (
                    <button
                      key={category.id}
                      className="hover: cursor-pointer w-full flex items-center gap-3 px-3 py-2 text-gray-300 hover:bg-zinc-800 rounded-lg transition-colors"
                    >
                      <ListTodo size={18} />
                      <span>{category.name}</span>
                      <div className="ml-auto flex items-center gap-2">
                        {category.count > 0 && (
                          <span className="text-sm text-white-500">{category.count}</span>
                        )}
                      </div>
                    </button>
                  ))}
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-black text-white border-t border-zinc-800 z-50">
          <div className="flex items-center justify-around p-4">
            <button
              onClick={() => handleNavigation('calendar', '/calendar')}
              className={`hover: cursor-pointer p-2 transition-colors ${activeSection === 'calendar' ? 'text-blue-400' : 'hover:text-blue-400'}`}
            >
              <Calendar size={24} />
            </button>
            <button
              onClick={() => handleNavigation('tasks', '/task')}
              className={`hover: cursor-pointer p-2 transition-colors ${activeSection === 'tasks' ? 'text-blue-400' : 'hover:text-blue-400'}`}
            >
              <CheckSquare size={24} />
            </button>
            <button
              onClick={() => handleNavigation('profile', '/profile')}
              className={`hover: cursor-pointer p-2 transition-colors ${activeSection === 'profile' ? 'text-blue-400' : 'hover:text-blue-400'}`}
            >
              <User size={24} />
            </button>
          </div>
